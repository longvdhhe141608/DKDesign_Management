...
public class EmployeeDao {
    ...
    public int updateProfile(Employee employee) {
        String sql = "UPDATE `dkmanagement`.`employees` " +
                "SET `name` = ?, `address` = ?, `avatar` = ?, `gender` = ?, `dob` = ?, `cccd` = ?, `email` = ?, `phone` = ?" +
                "WHERE (`id` = ?)";
        return jdbcTemplate.update(sql, employee.getName(), employee.getAddress(), employee.getAvatar(), employee.getGender(), employee.getDob(), employee.getCccd(), employee.getMail(), employee.getPhone(), employee.getId());
    }
